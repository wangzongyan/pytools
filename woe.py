...
def woe_iv(x, y, n = 10, detail = False):
	...
	out['woe'] = [math.log(bads / goods) if goods != 0 and bads != 0 else 1 for bads, goods in zip(out.percentage_bads, 
		out.percentage_goods)]
	out['iv'] = [(bads - goods) * woe for bads, goods, woe in zip(out.percentage_bads, 
...
def iv_rank(df, y, n = 10, min_split = 10, sorted = True):
	...
	"""
	out = pd.DataFrame({'var': list(df.columns), 
		'iv': np.repeat(np.nan, len(df.columns))}, index = range(len(df.columns)))
